...
train_filenames = [f[:-4]
                   for f in filter(lambda x: startswith(x, imgs_train), filenames)]

x_train_filenames = [os.path.join(datadir, 'crop', x+'.jpg')
                     for x in train_filenames]
y_train_filenames = []
for i in range(len(terms)):
    y_train_filenames.append(
        [os.path.join(datadir, str(terms[i]), x+'.png') for x in train_filenames])

val_filenames = [f[:-4]
                 for f in filter(lambda x: startswith(x, imgs_val), filenames)]

x_val_filenames = [os.path.join(datadir, 'crop', x+'.jpg')
                   for x in val_filenames]
y_val_filenames = []
for i in range(len(terms)):
    y_val_filenames.append(
        [os.path.join(datadir, str(terms[i]), x+'.png') for x in val_filenames])

test_filenames = [f[:-4]
                  for f in filter(lambda x: startswith(x, imgs_test), filenames)]

x_test_filenames = [os.path.join(datadir, 'crop', x+'.jpg')
                    for x in test_filenames]
y_test_filenames = []
for i in range(len(terms)):
    y_test_filenames.append(
        [os.path.join(datadir, str(terms[i]), x+'.png') for x in test_filenames])
...
def min_allowed_size(**conf):
    crop_size_conf = crop_size(
        conf["filters"], conf["dilations"], conf["conv"])
...
for iss in range(min_allowed_size(**model_conf)+min_output_size, min_allowed_size(**model_conf)+min_output_size*2, 2):
    ...
    try:
        ...
        model = tf.keras.Model(input_layer, unet(
            input_layer, len(params.terms), **model_conf))
...
def predict(model, dataset, img_id, img_index):
    res = tf.TensorArray(
        tf.uint8, size=0, dynamic_size=True, clear_after_read=False)
    i = 0
...
def make_images(res, img_id, img_index, min_output_size, output_shape, line, col, model_name, num_channels, mask_names):
    ...
    res = tf.image.crop_to_bounding_box(
        res, offset, offset, min_output_size, min_output_size)
    res = tf.reshape(res, (line, col, min_output_size,
                           min_output_size, num_channels))
    res = tf.transpose(res, (0, 2, 1, 3, 4))
    res = tf.reshape(res, (line*min_output_size, col *
                           min_output_size, num_channels))
...
for img_id in imgs_test:
    for img_index in [0, 1]:
        ...
        x_test_filenames_single = np.array(
            [s for s in filter(lambda x:prog.match(x) is not None, x_test_filenames)])
        y_test_filenames_single = np.array([[s for s in filter(lambda x:prog.match(
            x) is not None, y_test_filenames[:, i])] for i in range(num_mask)]).transpose()
        ...
        col = len(
            [s for s in filter(lambda x: prog1.match(x) is not None, test_filenames)])
        line = len(
            [s for s in filter(lambda x: prog2.match(x) is not None, test_filenames)])
        print(col, line)
        if col == 0 or line == 0:
            break
        mask_names = tf.constant(['fullmask_{}_{}_{}_{}.png'.format(
            model_conf['name'], img_id, img_index, i) for i in range(num_mask)])
        img_id = tf.constant(img_id)
        img_index = tf.constant(img_index)
        line = tf.constant(line)
        col = tf.constant(col)
        ds = get_masks(model, img_id, img_index,
                       x_test_filenames_single, y_test_filenames_single)
        res = predict(model, ds, img_id, img_index)
        make_images(res, img_id, img_index, min_output_size,
                    output_shape_1, line, col, model_name, num_mask_t, mask_names)
