...
def leg(input, concat, num_filters=1, name="leg"):
    ...
    upped = tf.keras.layers.Cropping2D(
        (upped.shape[1]-concat.shape[1])//2)(upped)
...
def unet(x, num_class,
    ...
    if filters is None:
        ...
        raise Exception('depth and filter do not match  {} {} {}'.format(
            filters, dilations, conv))

    if dilations is None:
        dilations = [1 for i in range(depth+1)]
    elif len(dilations) != depth+1:
        raise Exception('dilations and depth do not match  {} {} {}'.format(
            filters, dilations, conv))
    ...
    if len(convs) != depth+1:
        ...
        raise Exception('conv does not match depth {} {} {}'.format(
            filters, dilations, conv))
    ...
    for i in range(depth):
        conv = convs[i]
        encoder = []
        rescrops.append(tf.keras.layers.Cropping2D(
            (filters[i]-1)*dilations[i]//2*conv))
        for j in range(conv):
    ...
    i = depth
    if residuals:
        residual_conv_center = tf.keras.layers.Conv2D(
            filters_factor*2**(depth), 1, name='cen_res_conv')
        residual_conv_bn = tf.keras.layers.BatchNormalization()
        residual_conv_activation = tf.keras.layers.ELU()
        rescrops_center = tf.keras.layers.Cropping2D(
            (filters[i]-1)*dilations[i]//2*conv)
    ...
    for i in reversed(list(range(depth))):
        derescrops.append(tf.keras.layers.Cropping2D(
            (filters[i]-1)*dilations[i]//2*conv))
    ...
    final = tf.keras.layers.Conv2D(
        num_class, 1, activation='sigmoid', name='fin_conv')
    ###
    encoded = []
