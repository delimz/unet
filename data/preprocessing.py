...
def process_pathnames(fname, label_path):
    ...
    img = tf.truediv(tf.cast(tf.image.decode_jpeg(img_str, channels=3),dtype=tf.float32),255.0)
    ...
    label_img = tf.image.convert_image_dtype(label_img[:,:,:,0],dtype=tf.float32) 
...
def get_dataset(filenames,
    ...
    dataset = dataset.map(process_pathnames,num_parallel_calls=threads, deterministic=deterministic)
    # Map our preprocessing function to every element in our dataset, taking
    # advantage of multithreading
    if filter_empty:
        dataset=dataset.filter(filter_fn)

    dataset =dataset.map(preproc_fn, num_parallel_calls=threads, deterministic=deterministic)
...
def crop_strech_img(img,label_img, deformation_range,img_shape,orig_shape=1024):
    ...
    if deformation_range != 0:
        cx=tf.random.uniform([],x_deform/2+img_shape[0]/2,orig_shape-img_shape[0]/2-x_deform/2)
        cy=tf.random.uniform([],y_deform/2+img_shape[1]/2,orig_shape-img_shape[1]/2-y_deform/2)
    else:
        cx=orig_shape/2
        cy=orig_shape/2
    y1=cy-img_shape[0]/2
...
def augment(img,
    ...
    img, label_img = crop_strech_img(img,label_img,deformation_range,nn_input_shape,orig_shape)
    
    label_img = tf.image.central_crop(label_img,nn_output_shape[0]/nn_input_shape[0])
    if hue_delta:
