        ...
        if image.id in imgs.keys():
            ...
            for term in terms:
                ...
                try:
                    ...
                    print("no annotation with term {} in image {}".format(
                        term, image.id))
                    continue
            ...
            for b in bs:
                if os.path.exists(os.path.join(datadir, "{}_{}.jpg".format(image.id, num))):
                    ...
                    crop = img.read_region(
                        (b[0], img.dimensions[1]-b[3]), 0, (b[2]-b[0], b[3]-b[1]))
                    crop = crop.convert("RGB")
                    print("saving image {} {}".format(image.id, num))
                    crop.save(os.path.join(
                        datadir, "{}_{}.jpg".format(image.id, num)))
                ...
                for term in terms:
                    ...
                    for anot in terms_annot[term]:
                        # .format(sf,b[0]+b[2],anot)
                        sf = '{}<g transform="translate({},{}), scale(1,-1)">{}</g>'.format(
                            sf, 0, b[1]+b[3], anot)
                    sf = "{}{}".format(sf, "</svg>")
    ...
    for i in range(0, height, h//overlap):
        for j in range(0, width, w//overlap):
            if not os.path.exists(os.path.join(datadir, "crop", "{}_{}_{}_{}_{}.jpg".format(img, i, j, h, w))):
                if crop is None:
                    crop = cv2.imread(os.path.join(
                        datadir, "{}.jpg".format(img)))
                    print("loaded ", img, " with size ", crop.shape)
                cv2.imwrite(os.path.join(datadir, "crop", "{}_{}_{}_{}_{}.jpg".format(
                    img, i, j, h, w)), crop[i:i+h, j:j+w, :])

            for term in terms:
                if not os.path.exists(os.path.join(datadir, str(term), "{}_{}_{}_{}_{}.png".format(img, i, j, h, w))):
                    print("{}/{}/{}_{}_{}_{}_{}.png".format(datadir,
                                                            term, img, i, j, h, w))
                    try:
                        cv2.imwrite(os.path.join(datadir, str(term), "{}_{}_{}_{}_{}.png".format(
                            img, i, j, h, w)), masks[term][i:i+h, j:j+w, :])
                    except KeyError:
                        masks[term] = cv2.imread(os.path.join(
                            datadir, "{}_{}.png".format(img, term)))
                        cv2.imwrite(os.path.join(datadir, str(term), "{}_{}_{}_{}_{}.png".format(
                            img, i, j, h, w)), masks[term][i:i+h, j:j+w, :])

                else:
                    print(
                        "{}/{}/{}_{}_{}_{}_{}.png already".format(datadir, term, img, i, j, h, w))
    print("")
