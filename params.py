...
cytomine_params = [
    ...
    ("cytomine_public_key", None, str, [],
     'api public key for cytomine server'),
    ("cytomine_private_key", None, str, [],
     'api private key for the cytomine server'),
    ("cytomine_id_project", None, int, [],
     'project id on the cytomine server'),
    ("cytomine_id_software", None, int, [],
     'software id on the cytomine server'),
]
...
crop_params = [
    ...
    ("slice_term", None, int, -1,
     'id of the annotation delimiting the zone to segment'),
...
augmentations_params = [
    ...
    ("saturation_delta", None, float, 0.05,
     'data augmentation by staturation alteration range'),
    ("brightness_delta", None, float, 0.05,
     'data augmentation by brightness alteration range'),
    ("contrast_delta", None, float, 0.05,
     'data augmentation by contrast alteration range'),
    ("horizontal_flip", None, parsebool, True,
     'data augmentation by flipping images horizontally'),
    ("rotate_range", None, float, 15.0,
     'data augmentation by rotation angle range'),
    ("deformation_range", None, float, 0.02,
     'data augmentation by streching of the image'),
]
...
model_params = [
    ...
    ("filter_factor", None, int, 16,
     'base number of filters in first level of convolution'),
    ("dilations", '*', int, [1], 'size of dilation'),
    ("filter_sizes", '*', int, [3],
     'size of the filters for convolutions'),
    ...
    ("min_output_size", None, int, 512,
     'minimum size of output of the neural network'),
...
def parserer():
    ...
    for param, nargs, typef, default, helps in params:
        parser.add_argument("--{}".format(param), nargs=nargs,
                            default=default, type=typef, help=helps)
...
def parse_params(argv):
    ...
    if len(parsed.filter_sizes) == 1:
        parsed.filter_sizes = [parsed.filter_sizes[0]
    ...
    if len(parsed.convolutions) == 1:
        parsed.convolutions = [parsed.convolutions[0]
                               for i in range(parsed.depth+1)]
